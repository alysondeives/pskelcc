#include "jacobi3d19pt_kernel.hu"
__global__ void kernel0(float *A, float *B, int n, int tsteps, int c0)
{
    ...
    for (int c1 = 32 * b0; c1 < n - 3; c1 += 8192)
      if (n >= t0 + c1 + 4 && t0 + c1 >= 3)
        for (int c2 = 32 * b1; c2 < n - 3; c2 += 8192)
          for (int c3 = 0; c3 < n - 3; c3 += 32)
            for (int c5 = ppcg_max(t1, ((t1 + c2 + 1) % 4) - c2 + 3); c5 <= ppcg_min(31, n - c2 - 4); c5 += 4)
              for (int c6 = ppcg_max(t2, ((t2 + c3 + 1) % 4) - c3 + 3); c6 <= ppcg_min(31, n - c3 - 4); c6 += 4)
