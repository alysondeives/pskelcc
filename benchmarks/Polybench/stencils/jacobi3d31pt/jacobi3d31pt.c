...
void jacobi3d31pt(int tsteps, int x, int y, int z, DATA_TYPE *A, DATA_TYPE *B) {
    int t, i, j, k;
    ...
    for (t = 0; t < _PB_TSTEPS; t++) {
        for (i = RADIUS; i < _PB_Z - RADIUS; ++i) {
            for (j = RADIUS; j < _PB_Y - RADIUS; ++j) {
                for (k = RADIUS; k < _PB_X - RADIUS; ++k) {
                    B[i*(_PB_X * _PB_Y) + j*_PB_X + k] = c0 * A[(i+5)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +  c1 * A[(i-5)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +
														 c0 * A[(i+4)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +  c1 * A[(i-4)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +
														 c0 * A[(i+3)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +  c1 * A[(i-3)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +
														 c0 * A[(i+2)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +  c1 * A[(i-2)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +
														 c0 * A[(i+1)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +  c1 * A[(i-1)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +
														 c2 * A[(i  )*(_PB_X * _PB_Y) + (j+5)*_PB_X + (k  )]  +  c3 * A[(i  )*(_PB_X * _PB_Y) + (j-5)*_PB_X + (k  )]  +
														 c2 * A[(i  )*(_PB_X * _PB_Y) + (j+4)*_PB_X + (k  )]  +  c3 * A[(i  )*(_PB_X * _PB_Y) + (j-4)*_PB_X + (k  )]  +
														 c2 * A[(i  )*(_PB_X * _PB_Y) + (j+3)*_PB_X + (k  )]  +  c3 * A[(i  )*(_PB_X * _PB_Y) + (j-3)*_PB_X + (k  )]  +
														 c2 * A[(i  )*(_PB_X * _PB_Y) + (j+2)*_PB_X + (k  )]  +  c3 * A[(i  )*(_PB_X * _PB_Y) + (j-2)*_PB_X + (k  )]  +
														 c2 * A[(i  )*(_PB_X * _PB_Y) + (j+1)*_PB_X + (k  )]  +  c3 * A[(i  )*(_PB_X * _PB_Y) + (j-1)*_PB_X + (k  )]  +
														 c4 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k+5)]  +	 c5 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k-5)]  +
													  	 c4 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k+4)]  +	 c5 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k-4)]  +
													  	 c4 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k+3)]  +	 c5 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k-3)]  +
													  	 c4 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k+2)]  +	 c5 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k-2)]  +
														 c4 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k+1)]  +	 c5 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k-1)]  +
														 c6 * A[(i  )*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )];
                }
            }
        }
        ...
        for (j = 0; j < y; ++j) {
            for (k = 0; k < x; ++k) {
				if (i<offset_z || j<offset_y || i>=z-offset_z || j>=y-offset_y || k<offset_x || k>=x-offset_x)
					...
					A[i * (x * y) + j * x + k] = (DATA_TYPE)  (i + j + (x-k))* 10 / (x); //i % 12 + 2 * (j % 7) + 3 * (k % 13);
...
int main(int argc, char *argv[]) {
    ...
    t_start = rtclock();
    jacobi3d31pt(tsteps, x+2*RADIUS, y+2*RADIUS, z+2*RADIUS, A, B);
    t_end = rtclock();
    ...
    init(A,X+2*RADIUS,Y+2*RADIUS,Z+2*RADIUS,RADIUS,RADIUS,RADIUS);
    t_start = rtclock();
    jacobi3d31pt_GPU_baseline(tsteps, z, y, x, A, B_GPU);
    t_end = rtclock();
    ...
    t_start = rtclock();
    jacobi3d31pt_GPU_opt(tsteps, z, y, x, A_GPU_OPT, B_GPU_OPT);
    t_end = rtclock();
