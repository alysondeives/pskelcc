...
void jacobi3d7pt(int tsteps, int x, int y, int z, DATA_TYPE *A, DATA_TYPE *B) {
    int t, i, j, k;
    ...
    for (t = 0; t < _PB_TSTEPS; t++) {
        for (i = 1; i < _PB_Z - 1; ++i) {
            for (j = 1; j < _PB_Y - 1; ++j) {
                for (k = 1; k < _PB_X - 1; ++k) {
                    B[i*(_PB_X * _PB_Y) + j*_PB_X + k] = c0 * A[(i+1)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +  c1 * A[(i-1)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +
...
int main(int argc, char *argv[]) {
    ...
    t_start = rtclock();
    jacobi3d7pt(tsteps, x+2*RADIUS, y+2*RADIUS, z+2*RADIUS, A, B);
    t_end = rtclock();
    ...
    init(A,X+2*RADIUS,Y+2*RADIUS,Z+2*RADIUS,RADIUS,RADIUS,RADIUS);
    t_start = rtclock();
    jacobi3d7pt_GPU_baseline(tsteps, z, y, x, A, B_GPU);
    t_end = rtclock();
    ...
    t_start = rtclock();
    jacobi3d7pt_GPU_opt(tsteps, z, y, x, A_GPU_OPT, B_GPU_OPT);
    t_end = rtclock();
