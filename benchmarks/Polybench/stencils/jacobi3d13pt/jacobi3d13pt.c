...
void jacobi3d13pt(int tsteps, int x, int y, int z, DATA_TYPE *A, DATA_TYPE *B) {
    int t, i, j, k;
        ...
        for (i = RADIUS; i < _PB_Z - RADIUS; ++i) {
            for (j = RADIUS; j < _PB_Y - RADIUS; ++j) {
                for (k = RADIUS; k < _PB_X - RADIUS; ++k) {
                    B[i*(_PB_X * _PB_Y) + j*_PB_X + k] = c0 * A[(i+2)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +  c1 * A[(i-2)*(_PB_X * _PB_Y) + (j  )*_PB_X + (k  )]  +
...
int main(int argc, char *argv[]) {
    ...
    t_start = rtclock();
    jacobi3d13pt(tsteps, x+2*RADIUS, y+2*RADIUS, z+2*RADIUS, A, B);
    t_end = rtclock();
    ...
    init(A,X+2*RADIUS,Y+2*RADIUS,Z+2*RADIUS,RADIUS,RADIUS,RADIUS);
    t_start = rtclock();
    jacobi3d13pt_GPU_baseline(tsteps, z, y, x, A, B_GPU);
    t_end = rtclock();
    ...
    t_start = rtclock();
    jacobi3d13pt_GPU_opt(tsteps, z, y, x, A_GPU_OPT, B_GPU_OPT);
    t_end = rtclock();
