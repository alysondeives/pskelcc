    ...
    for (t = 0; t < _PB_TSTEPS; t++){
        ...
        for (k = 1; k < _PB_NK -1; ++k){
            B[i*(_PB_NK * _PB_NJ) + j*_PB_NK + k] = c0 * A[(i  )*(_PB_NK * _PB_NJ) + (j  )*_PB_NK + (k-1)]  +
                                                    ...
                                                    c3 * A[(i-1)*(_PB_NK * _PB_NJ) + (j-1)*_PB_NK + (k  )]  +
                                                    c4 * A[(i-1)*(_PB_NK * _PB_NJ) + (j+1)*_PB_NK + (k  )]  +
                                                    c5 * A[(i+1)*(_PB_NK * _PB_NJ) + (j-1)*_PB_NK + (k  )]  +
                                                    c6 * A[(i+1)*(_PB_NK * _PB_NJ) + (j+1)*_PB_NK + (k  )];
        ...
        for (i = 1; i < _PB_NI - 1; ++i){
        for (k = 1; k < _PB_NK -1; ++k){
            B[i*(_PB_NK * _PB_NJ) + j*_PB_NK + k] = A[i*(_PB_NK * _PB_NJ) + j*_PB_NK + k];
        }
        }
>>>>>>> conflict
        }
...
int main(int argc, char *argv[])
{
  ...
  fprintf(stdout, ">> Three dimensional (3D) jacobi <<\n");
